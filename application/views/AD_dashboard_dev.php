        	...
        	<tr>
            	<td class="lftHeadCol">
            		<table width="100%" border="0" cellspacing="0" cellpadding="10">
            			<tr>
                        	<td><h3><strong>Dealer Surveys</strong></h3></td>
                            <td align="right" style="font-size:110%">
                                <table border="0" align="right" cellpadding="2" cellspacing="0">
                                    <tr>
                                        <td nowrap="nowrap"><a href="/media/dealer_survey.pdf" onclick="basicPopup(this.href);return false">Printable Survey</a></td>
                                        <td width="1" nowrap="nowrap">|</td>
                                        <td nowrap="nowrap"><a href="<?=$this->config->item('base_url')?>index.php/survey/dealer_survey/create/<?=$adInfo[0]->adID?>">Submit a Survey</a></td>
<?php 
	if($dealerSurveys){
?>
                                        <td width="1" nowrap="nowrap">|</td>
                                        <td nowrap="nowrap">
                    <select onchange="window.location=this.options[this.selectedIndex].value;" style="min-width:300px">
                        <option value="">------&nbsp;View Dealership Survey&nbsp;------</option>
<?php 
		//$prevYear = NULL;
		foreach($dealerSurveys as $survey){
			/*if($prevYear != $oldRpt->year){
			 	echo ' <optgroup label="'.$oldRpt->year.'">';
				$prevYear = $oldRpt->year;	
			}*/
?>
                        <option value="<?=$this->config->item('base_url').'index.php/survey/dealer_survey/view/'.$adInfo[0]->adID.'/'.$survey->ID?>"><?=$survey->dealerName?></option>
<?php 
		}
?>                   
                    </select>
...
//echo $this->functions->debug($dealerSurveys);
