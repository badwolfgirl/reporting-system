...
<?=form_open('reports/monthly_AD_sales/edit/'.$areadev[0]->adID.'/'.$month.'/'.$year, $attributes);?>
			...
			<tr>
				...
				<h2><?=$mainh2?> 
						<select name="month" id="month" style="width:150px;">
							<option value="<?=$month?>"><?=date('F', mktime(0, 0, 0, $month, date("d"), date("Y")))?></option>
						</select>
...
?>					
					...
					<tr>
						...
						<td class="centHeadCol"># of<br />Units</td>
					...
					<?php 
					if($reports[0] != false){
					foreach($reports as $key => $fAD){ 
						if($fAD->franID == $areadev[0]->franID){
					...
					<tr class="salesrow">
						<td class="lftFormCol"><strong><?=$fAD->firstName.' '.$fAD->lastName?></strong><br /><?=$areadev[0]->adID.' - '.$fAD->franID?>
							...
							<input name="frans[<?=$key?>][ID]" type="hidden" value="<?=$fAD->ID?>">
						</td>
						<td class="centFormCol"><input name="frans[<?=$key?>][units]" class="units" type="text" size="4" value="<?php if(isset($_POST['frans'][$key]['units'])) echo $_POST['frans'][$key]['units']; elseif(isset($fAD->units)) echo $fAD->units; else echo '0'; ?>" title="# of Units"></td>
					...
					<?php 
						...
						if(isset($key)) $newKey = $key+1; else $newKey = 1;
							
						foreach($reports as $key => $fran){ 
						if($fran->franID != $areadev[0]->franID){
						
							if($newKey % 2 != 0) $sty = '_odd'; else $sty = '';
					...
					<tr class="salesrow">
						<td class="lftFormCol<?=$sty?>"><strong><?=$fran->firstName.' '.$fran->lastName?></strong><br /><?=$areadev[0]->adID.' - '.$fran->franID?>
							...
							<input name="frans[<?=$newKey?>][ID]" type="hidden" value="<?=$fran->ID?>">
						</td>
						<td class="centFormCol<?=$sty?>"><input name="frans[<?=$newKey?>][units]" class="units" type="text" size="4" value="<?php if(isset($_POST['frans'][$key]['units'])) echo $_POST['frans'][$key]['units']; elseif(isset($fran->units)) echo $fran->units; else echo '0'; ?>" title="# of Units"></td>
		...
		<tr>
			<td><table cellpadding="10" cellspacing="2" width="100%" style="border:1px solid #ccc">
				...
				<tr>
					<td class="lftFormCol"><h3>Special comments for email here:</h3><textarea name="comments" style="width:99%;" rows="2"><?php if($notes) echo $notes; else echo '';?></textarea></td>
				</tr>
				<tr>
					<td class="lftFormCol"><div class="note"><p><strong>Note: </strong> New Techs should not begin training until they have SIGNED a franchise agreement or an employee non-competition agreement. <br /> Include employee sales in their franchisee/employer sales.</p></div>
						<div class="buttons"><input name="reset" type="button" onClick="window.location.href=window.location.href" value="Reset" />&nbsp;&nbsp;|&nbsp;&nbsp;<input type="submit" name="action" value="Save Form" />&nbsp;&nbsp;|&nbsp;&nbsp;<input type="submit" name="action" value="<?=$button?>" />
